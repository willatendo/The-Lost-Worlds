...
public class ModOreFeature extends Feature<ModOreFeatureConfig> {
	public ModOreFeature(Codec<ModOreFeatureConfig> config) {
		super(config);
	}
	...
	protected boolean doPlace(LevelAccessor world, Random rand, ModOreFeatureConfig config, double p_207803_4_, double p_207803_6_, double p_207803_8_, double p_207803_10_, double p_207803_12_, double p_207803_14_, int p_207803_16_, int p_207803_17_, int p_207803_18_, int p_207803_19_, int p_207803_20_) {
		int i = 0;
									...
									if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D) {
										...
										if (!bitset.get(l2)) {
											...
											if (config.target.test(world.getBlockState(blockpos$mutable), rand)) {
												world.setBlock(blockpos$mutable, config.state.get(), 2);
												++i;
											}
										}
									}
