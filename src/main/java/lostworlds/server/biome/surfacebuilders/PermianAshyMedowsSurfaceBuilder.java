...
public class PermianAshyMedowsSurfaceBuilder extends SurfaceBuilder<SurfaceBuilderBaseConfiguration> {
	public PermianAshyMedowsSurfaceBuilder(Codec<SurfaceBuilderBaseConfiguration> codec) {
		super(codec);
	}
	...
	public void apply(Random rand, ChunkAccess iChunk, Biome biome, int x, int z, int startHeight, double noise, BlockState block, BlockState fluid, int seaLevel, long seed, SurfaceBuilderBaseConfiguration config) {
		if (noise > 1.0D) {
			...
			SurfaceBuilder.DEFAULT.apply(rand, iChunk, biome, x, z, startHeight, noise, block, fluid, seaLevel, seed, ModSurfaceBuilderConfigs.VOLCANIC_ASH_CONFIG.get());
		}
	}
}
