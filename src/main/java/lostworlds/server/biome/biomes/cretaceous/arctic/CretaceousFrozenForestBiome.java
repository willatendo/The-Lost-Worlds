...
public class CretaceousFrozenForestBiome extends ModBiome {
	private float depth;
	private float scale;

	public CretaceousFrozenForestBiome(float depth, float scale) {
		this.depth = depth;
		this.scale = scale;
	}

	public CretaceousFrozenForestBiome() {
		this(0.125F, 0.05F);
	}

	@Override
	public Biome getBiome() {
		return this.biome(Precipitation.SNOW, BiomeCategory.FOREST, this.depth, this.scale, 0.5F, 0.0F, this.ambience(), this.generation(), this.spawn()).build();
	}
