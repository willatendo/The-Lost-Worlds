...
public class CretaceousPlainsBiome extends ModBiome {
	private float depth;
	private float scale;

	public CretaceousPlainsBiome(float depth, float scale) {
		this.depth = depth;
		this.scale = scale;
	}

	public CretaceousPlainsBiome() {
		this(0.125F, 0.05F);
	}

	@Override
	public Biome getBiome() {
		return this.biome(RainType.RAIN, Category.PLAINS, this.depth, this.scale, 0.8F, 0.7F, this.ambience(), this.generation(), this.spawn()).build();
	}

	@Override
	public BiomeGenerationSettings generation() {
		BiomeGenerationSettings.Builder generation = new BiomeGenerationSettings.Builder();
		generation.surfaceBuilder(() -> LostWorldsSurfaceBuilders.NAKED_CRETACEOUS_PLAINS.configured(ModSurfaceBuilderConfigs.PODZOL_CONFIG.get()));
