...
public class LostWorldsRegistrate extends AbstractRegistrate<LostWorldsRegistrate> {
	...
	public void addBooks() {
		this.addDataGenerator(BOOKS, provider -> provider.addBook(new BookBuilder("field_guide", "Field Guide", "By Willatendo", 4546862).addSections(new SectionBuilder("modern", AncientCreatures.GREAT_AUK.getSkeletonPick().get()).addPages(new TextPageBuilder("great_auk").addTitle("Pinguinus impennis - Great Auk").addFirst("The Great Auk, a large auk from the Neogene to Holocene. This large auk spaned across the North Pacific ocean. It had the odd predators like Polar Bears and Humans.").addNext("The Great Auk was 80 centimeters tall. It would've hunted fish. Humans traded thier parts all across North America and Europe.")).addPages(new TextPageBuilder("greak_auk_info").addFirst("Danger: Minimal").addNext("Diet: Piscivore").addNext("Range: Neogene - Late Holocene").addNext("Order: Charadriiformes").addNext("Family: Alcidae").addNext("Genus: Pinguinus"))).addSections(new SectionBuilder("cretaceous", AncientCreatures.TYRANNOSAURUS.getSkeletonPick().get())
				/* Carnotaurus */
