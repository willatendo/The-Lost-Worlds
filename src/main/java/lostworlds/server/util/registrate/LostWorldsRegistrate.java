...
public class LostWorldsRegistrate extends AbstractRegistrate<LostWorldsRegistrate> {
	...
	public void copyTag(ITag.INamedTag<Block> modBlockTag, ITag.INamedTag<Item> modItemTag) {
		this.addDataGenerator(ProviderType.ITEM_TAGS, provider -> provider.copy(modBlockTag, modItemTag));
	}

	public void add(ITag.INamedTag<Item> modItemTag, Item... items) {
		this.addDataGenerator(ProviderType.ITEM_TAGS, provider -> provider.tag(modItemTag).add(items));
	}

	public void blockAdd(ITag.INamedTag<Block> modBlockTag, Block... blocks) {
		this.addDataGenerator(ProviderType.BLOCK_TAGS, provider -> provider.tag(modBlockTag).add(blocks));
	}

	public void addTags(ITag.INamedTag<Block> modItemTag, ITag.INamedTag<Block>... tags) {
		this.addDataGenerator(ProviderType.BLOCK_TAGS, provider -> provider.tag(modItemTag).addTags(tags));
	}
