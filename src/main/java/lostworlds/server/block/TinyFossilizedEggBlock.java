...
public class TinyFossilizedEggBlock extends Block {
	...
	public TinyFossilizedEggBlock(AbstractBlock.Properties properties) {
		super(properties);
	...
	public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity entity, Hand hand, BlockRayTraceResult result) {
		if (entity.getItemInHand(hand) != null) {
			...
			if (item instanceof WetPaperItem) {
				world.setBlockAndUpdate(pos, LostWorldsBlocks.TINY_PLASTERED_FOSSILISED_EGG.defaultBlockState().setValue(EGGS, state.getValue(EGGS)));
