...
public class CalamtiesSuckowiiSaplingBlock extends Block implements IGrowable
{
	...
	public BlockState updateShape(BlockState state, Direction direction, BlockState newState, IWorld world, BlockPos pos, BlockPos newPos) 
	{
		...
		{
			if(direction == Direction.UP && newState.is(LostWorldsBlocks.CALAMITES_SUCKOWII)) 
			{
				world.setBlock(pos, LostWorldsBlocks.CALAMITES_SUCKOWII.defaultBlockState(), 2);
			}
	...
	{
		return new ItemStack(LostWorldsBlocks.CALAMITES_SUCKOWII.asItem());
	}
	...
	{
		world.setBlock(pos.above(), LostWorldsBlocks.CALAMITES_SUCKOWII.defaultBlockState().setValue(CalamtiesSuckowiiBlock.LEAVES, BambooLeaves.SMALL), 3);
	}
}
