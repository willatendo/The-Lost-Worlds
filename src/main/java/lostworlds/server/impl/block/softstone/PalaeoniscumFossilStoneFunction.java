...
public class PalaeoniscumFossilStoneFunction implements APISoftStoneFunction {
	@Override
	public void doFunction(BlockState state, ServerLevel world, BlockPos pos, ItemStack stack) {
		...
		if (creature == AncientCreatures.PALAEONISCUM) {
			if (EnchantmentHelper.getItemEnchantmentLevel(LostWorldsEnchantments.CURSE_OF_BREAKING.get(), stack) == 1) {
				...
				if (breakchance != 0) {
					Block.popResource(world, pos, AncientCreatures.PALAEONISCUM.getSkeletonPick().get().asItem().getDefaultInstance());
