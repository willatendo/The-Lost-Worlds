...
public class TimeMachineMenu extends AbstractContainerMenu {
	...
	public final Container container = new SimpleContainer(2) {
		public void setChanged() {
			super.setChanged();
			TimeMachineMenu.this.slotsChanged(this);
			TimeMachineMenu.this.slotUpdateListener.run();
		}
	};
	private final ResultContainer resultContainer = new ResultContainer();

	public TimeMachineMenu(MenuType<? extends TimeMachineMenu> type, int windowId, Inventory inventory, FriendlyByteBuf buffer) {
		this(type, windowId, inventory, ContainerLevelAccess.NULL);
	}

	public TimeMachineMenu(MenuType<? extends TimeMachineMenu> type, int windowId, Inventory inventory, ContainerLevelAccess containerLevelAccess) {
		super(type, windowId);
		...
		this.resultSlot = this.addSlot(new Slot(this.resultContainer, 2, 143, 33) {
			...
			public void onTake(Player player, ItemStack stack) {
				...
				TimeMachineMenu.this.resultContainer.awardUsedRecipes(player);
				ItemStack book = TimeMachineMenu.this.bookSlot.remove(1);
				ItemStack power = TimeMachineMenu.this.powerSlot.remove(1);
				if (!book.isEmpty() && !power.isEmpty()) {
					TimeMachineMenu.this.setupResultSlot();
				}

				containerLevelAccess.execute((world, pos) -> {
					long l = world.getGameTime();
					if (TimeMachineMenu.this.lastSoundTime != l) {
						world.playSound((Player) null, pos, SoundEvents.BEACON_ACTIVATE, SoundSource.BLOCKS, 1.0F, 1.0F);
						TimeMachineMenu.this.lastSoundTime = l;
					}
				});
			}
		});
	...
	public boolean stillValid(Player entity) {
		return stillValid(this.containerLevelAccess, entity, LostWorldsBlocks.TIME_MACHINE.get());
	}
	...
	private void setupRecipeList(Container inv, ItemStack stack) {
		...
		if (!inv.isEmpty()) {
			this.recipes = this.level.getRecipeManager().getRecipesFor(LostWorldsRecipeTypes.TIME_MACHINE_RECIPE.get(), inv, this.level);
		}
	}
	...
	public MenuType<?> getType() {
		return LostWorldsMenus.TIME_MACHINE_CONTAINER.get();
	}
