...
public class JurassicChunkGenerator extends NoiseBasedChunkGenerator {
	public static final Codec<JurassicChunkGenerator> CODEC = RecordCodecBuilder.create((p_188643_) -> {
		...
		}))).apply(p_188643_, p_188643_.stable(JurassicChunkGenerator::new));
	});

	public JurassicChunkGenerator(Registry<StructureSet> structures, Registry<NormalNoise.NoiseParameters> noises, BiomeSource source, long seed, Holder<NoiseGeneratorSettings> settings) {
	...
	public ChunkGenerator withSeed(long seed) {
		return new JurassicChunkGenerator(this.structureSets, this.noises, biomeSource.withSeed(WorldSeedHolder.getSeed()), WorldSeedHolder.getSeed(), this.settings);
	}
}
