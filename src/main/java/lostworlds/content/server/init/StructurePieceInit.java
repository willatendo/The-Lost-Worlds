...
public class StructurePieceInit 
{
	...
	public static void registerBiomeGeneration()
	{
		...
		WorldGenRegistries.NOISE_GENERATOR_SETTINGS.forEach(settings -> 
		{
			...
			settings.structureSettings().structureConfig().put(StructureInit.SURFACE_FOSSIL, new StructureSeparationSettings(32, 8, LostWorldsConfig.COMMON_CONFIG.surfaceFossilGenerationId.get()));
			settings.structureSettings().structureConfig().put(StructureInit.SUBTERRANEAN_FOSSIL, new StructureSeparationSettings(32, 8, LostWorldsConfig.COMMON_CONFIG.subterraneanFossilGenerationId.get()));
