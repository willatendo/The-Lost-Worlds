						...
						for(int j4 = 4; j4 < 8; ++j4) 
						{
							if(aboolean[(i2 * 16 + j3) * 8 + j4]) 
							{
								...
								if(isDirt(reader.getBlockState(blockpos).getBlock()) && reader.getBrightness(LightType.SKY, pos.offset(i2, j4, j3)) > 0) 
								{
									...
									if(biome.getGenerationSettings().getSurfaceBuilderConfig().getTopMaterial().is(BlockInit.PERMIAN_SAND)) 
									{
										reader.setBlock(blockpos, BlockInit.PERMIAN_SAND.defaultBlockState(), 2);
									} 
									else 
									{
										reader.setBlock(blockpos, BlockInit.MOSSY_SOIL.defaultBlockState(), 2);
									}
								}
							}
						}
					...
					for(int j2 = 0; j2 < 16; ++j2) 
					{
						for(int k3 = 0; k3 < 16; ++k3) 
						{
							for(int k4 = 0; k4 < 8; ++k4) 
							{
								...
								{
									reader.setBlock(pos.offset(j2, k4, k3), BlockInit.PERMIAN_STONE.defaultBlockState(), 2);
								}
							}
						}
					}
