...
{
	private static final BlockState CALAMITES_SUCKOWII_TRUNK = BlockInit.CALAMITES_SUCKOWII.defaultBlockState().setValue(BambooBlock.AGE, Integer.valueOf(1)).setValue(BambooBlock.LEAVES, BambooLeaves.NONE).setValue(BambooBlock.STAGE, Integer.valueOf(0));
	...
	public boolean place(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, ProbabilityConfig config) 
	{
		...
		{
			if(BlockInit.CALAMITES_SUCKOWII.defaultBlockState().canSurvive(reader, blockpos$mutable)) 
			{
