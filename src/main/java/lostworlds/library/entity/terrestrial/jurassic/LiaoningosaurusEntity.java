...
public class LiaoningosaurusEntity extends HerbivoreEggLayingEntity
{
	...
	private <E extends ITyrannomatable> PlayState predicate(TyrannomationEvent<E> event) 
	{
		if(event.isMoving())
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.liaoningosaurus.walk", true));
			return PlayState.CONTINUE;
		}
		else if(this.entityData.get(this.EATING))
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.liaoningosaurus.eat", false));
			return PlayState.CONTINUE;
		}
		else if(this.entityData.get(this.SLEEPING))
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.liaoningosaurus.into_sleep", false).addAnimation("animation.liaoningosaurus.sleeping", false).addAnimation("animation.liaoningosaurus.out_of_sleep", false));
			return PlayState.CONTINUE;
		}
		else
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.liaoningosaurus.idle", true));
			return PlayState.CONTINUE;
		}
	}
	...
	protected void registerGoals() 
	{
		...
		this.goalSelector.addGoal(6, new HerbivoreLayEggGoal(this, 1.0D, DinoTypes.LIAONINGOSAURUS));
	...
	{
		return EntityInit.LIAONINGOSAURUS.create(world);
	}
}
