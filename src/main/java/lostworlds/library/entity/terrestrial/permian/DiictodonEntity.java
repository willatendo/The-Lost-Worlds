...
import lostworlds.library.entity.goal.terrestrial.herbivore.entity.DiictodonCreateTerritoryGoal;
import lostworlds.library.entity.goal.terrestrial.herbivore.entity.DiictodonLayEggGoal;
...
public class DiictodonEntity extends HerbivoreEggLayingEntity
{
	...
	private <E extends ITyrannomatable> PlayState predicate(TyrannomationEvent<E> event) 
	{
		if(event.isMoving())
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.diictodon.walk", true));
			return PlayState.CONTINUE;
		}
		else if(this.entityData.get(this.EATING))
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.diictodon.eat", false));
			return PlayState.CONTINUE;
		}
		else if(this.entityData.get(this.SLEEPING))
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.diictodon.into_sleep", false).addAnimation("animation.diictodon.sleeping", false).addAnimation("animation.diictodon.out_of_sleep", false));
			return PlayState.CONTINUE;
		}
		else
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.diictodon.idle", true));
			return PlayState.CONTINUE;
		}
	}
