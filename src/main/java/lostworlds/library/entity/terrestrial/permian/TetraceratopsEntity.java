...
public class TetraceratopsEntity extends HerbivoreEggLayingEntity
{
	...
	private <E extends ITyrannomatable> PlayState predicate(TyrannomationEvent<E> event) 
	{
		if(event.isMoving())
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.tetraceratops.walk", true));
			return PlayState.CONTINUE;
		}
		else if(this.entityData.get(this.EATING))
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.tetraceratops.eat", false));
			return PlayState.CONTINUE;
		}
		else if(this.entityData.get(this.SLEEPING))
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.tetraceratops.into_sleep", false).addAnimation("animation.tetraceratops.sleeping", false).addAnimation("animation.tetraceratops.out_of_sleep", false));
			return PlayState.CONTINUE;
		}
		else
		{
			event.getController().setAnimation(new TyrannomationBuilder().addAnimation("animation.tetraceratops.idle", true));
			return PlayState.CONTINUE;
		}
	}

	public TetraceratopsEntity(EntityType<? extends TetraceratopsEntity> entity, World world) 
	{
		super(entity, world);
	}
	...
	protected void registerGoals() 
	{
		...
		this.goalSelector.addGoal(6, new HerbivoreLayEggGoal(this, 1.0D, DinoTypes.TETRACERATOPS));
