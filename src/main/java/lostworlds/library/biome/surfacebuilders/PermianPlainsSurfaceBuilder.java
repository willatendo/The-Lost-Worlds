...
public class PermianPlainsSurfaceBuilder extends SurfaceBuilder<SurfaceBuilderConfig> 
{
	public PermianPlainsSurfaceBuilder(Codec<SurfaceBuilderConfig> codec) 
	{
		super(codec);
	}
	...
	public void apply(Random rand, IChunk iChunk, Biome biome, int i1, int i2, int i3, double d, BlockState state1, BlockState state2, int i4, long l, SurfaceBuilderConfig config) 
	{
		if(d > 1.0D) 
		{
			SurfaceBuilder.DEFAULT.apply(rand, iChunk, biome, i1, i2, i3, d, state1, state2, i4, l, ModSurfaceBuilders.MOSSY_SOIL_CONFIG);
		}
		else
		{
