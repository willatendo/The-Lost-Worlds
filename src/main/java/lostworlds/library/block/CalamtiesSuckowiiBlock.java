...
public class CalamtiesSuckowiiBlock extends Block implements IGrowable
{
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) 
	{
		...
		else 
		{
			...
			if(blockstate.is(LostWorldsTags.ModBlockTags.CALAMITES_PLACEABLES)) 
			{
				...
				else if(blockstate.is(BlockInit.CALAMITES_SUCKOWII)) 
				{
				...
				else 
				{
					...
					return !blockstate1.is(BlockInit.CALAMITES_SUCKOWII) && !blockstate1.is(BlockInit.CALAMITES_SUCKOWII_SAPLING) ? BlockInit.CALAMITES_SUCKOWII_SAPLING.defaultBlockState() : this.defaultBlockState().setValue(AGE, blockstate1.getValue(AGE));
				}
			} 
			else 
			{
	...
	public BlockState updateShape(BlockState state, Direction direction, BlockState newState, IWorld world, BlockPos pos, BlockPos newPos) 
	{
		...
		if(direction == Direction.UP && newState.is(BlockInit.CALAMITES_SUCKOWII) && newState.getValue(AGE) > state.getValue(AGE)) 
		{
	...
	protected void growPlant(BlockState state, World world, BlockPos pos, Random rand, int age) 
	{
		...
		if(age >= 1) 
		{
			if(blockstate.is(BlockInit.CALAMITES_SUCKOWII) && blockstate.getValue(LEAVES) != BambooLeaves.NONE) 
			{
				if(blockstate.is(BlockInit.CALAMITES_SUCKOWII) && blockstate.getValue(LEAVES) != BambooLeaves.NONE)
				{
					bambooleaves = BambooLeaves.LARGE;
					if(blockstate1.is(BlockInit.CALAMITES_SUCKOWII)) 
					{
		...
		int i = state.getValue(AGE) != 1 && !blockstate1.is(BlockInit.CALAMITES_SUCKOWII) ? 0 : 1;
	...
	{
		int i;
		for(i = 0; i < 16 && reader.getBlockState(pos.above(i + 1)).is(BlockInit.CALAMITES_SUCKOWII); ++i) { }
		return i;
	}
	...
	{
		int i;
		for(i = 0; i < 16 && reader.getBlockState(pos.below(i + 1)).is(BlockInit.CALAMITES_SUCKOWII); ++i) { }
		return i;
	}
}
