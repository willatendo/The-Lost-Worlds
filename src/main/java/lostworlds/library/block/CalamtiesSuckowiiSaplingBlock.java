...
public class CalamtiesSuckowiiSaplingBlock extends Block implements IGrowable
{
	...
	public BlockState updateShape(BlockState state, Direction direction, BlockState newState, IWorld world, BlockPos pos, BlockPos newPos) 
	{
		...
		{
			if(direction == Direction.UP && newState.is(BlockInit.CALAMITES_SUCKOWII)) 
			{
				world.setBlock(pos, BlockInit.CALAMITES_SUCKOWII.defaultBlockState(), 2);
			}
	...
	{
		return new ItemStack(BlockInit.CALAMITES_SUCKOWII.asItem());
	}
	...
	{
		world.setBlock(pos.above(), BlockInit.CALAMITES_SUCKOWII.defaultBlockState().setValue(CalamtiesSuckowiiBlock.LEAVES, BambooLeaves.SMALL), 3);
	}
}
